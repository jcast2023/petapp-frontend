...
@Component({
  ...
  styleUrl: './crear-peso.component.scss'
})
export class CrearPesoComponent implements OnInit {
  ...
  formulario = this.fb.group({
    ...
    pesoKg: ['', [Validators.required, Validators.min(0.1)]],
  ...
  onSubmit(): void {
    ...
    console.log('📤 Creando registro de peso:', datos);

    this.pesoService.crear({
      fecha: datos.fecha!,
    ...
    .subscribe({
      ...
      error: (error: HttpErrorResponse) => {
        console.error('❌ Error creando registro de peso:', error);
        this.error.set(error.error?.mensaje || 'Error al crear el registro de peso');
      }
    });
  }
}
